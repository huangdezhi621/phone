...
_e(function (E, $) {
    ...
    Menu.fn.bindEvent = function () {
        ...
        $domNormal.click(function (e) {
            if (!self.disabled()) {
            	if(self.id=='img'){
                if(self.editor.config.imagebox){
                  material(self.editor.config.imagebox).then(function (val) {
                    var imgHtml = '<img style="max-width:100%;" src="' + val[0].url + '"/>';
                    self.editor.command(e, 'insertHtml', imgHtml);
                    self.editor.confirm && self.editor.confirm(val);
                  }).catch(function (error) {
                    self.editor.close && self.editor.close(error);
                  });
                }else{
                  self.editor.config.material.call(this).then(val => {
                    self.editor.command(e, 'insertHtml', '<img style="max-width:100%;" src="' + val[0].url + '"/>');
                    self.editor.confirm && self.editor.confirm(val);
                  }).catch(function (error) {
                    self.editor.close && self.editor.close(error);
                  });
                  ...
                  //   imageboxUrl: self.editor.config.imageboxUrl,
                  //   width: 820,         //宽度
                  //   height: 620,        //高度
...
_e(function (E, $) {
    ...
    E.langs.en = {
        ...
        dynamicMap: 'Dynamic',
        clearLocation: 'Clear',
        langDynamicOneLocation: 'Only one location in dynamic mapNav',
...
_e(function (E, $) {
    ...
    //表情服务器
    E.config.emotionHref = 'https://duofriend.com/images/activity/miniscene/phone/face/';
    ...
    // 百度地图的key
    E.config.mapAk = 'ep4U9yfgijUmA2zRMjf25Tf6ix96nPvV';
...
_e(function (E, $) {
    ...
    E.fn.removeMenus = function(){

        var editor = this;
        var menuIds = editor.config.menus;
        var nonMenus = editor.config.nonMenus;

        var numIdx = 0;
        $.each(nonMenus, function (k,v) {
            numIdx = menuIds.indexOf(v);
            if(numIdx > -1){
                menuIds.splice(numIdx,1);
            }
        });
    }
...
_e(function (E, $) {
    ...
    E.createMenu(function (check) {
        ...
        var href = editor.config.emotionHref;
        ...
        $.each(configEmotions, function (k, val) {
            ...
            $.each(imgs, function (k, src) {
                var $command = $('<a href="#" commandValue="' + href + src + '"></a>');
                var $img = $('<img>');
                $img.css({
                    width: size + 'px',
                    height: size + 'px'
                });
                $img.attr('_src', href + src);  // 先将 src 复制到 '_src' 属性，先不加载
...
_e(function (E, $) {
    E.createMenu(function (check) {
        ...
        //https://v.qq.com/iframe/player.html?vid=q0377jakyef&tiny=0&auto=0
        var editor = this;
        var lang = editor.config.lang;
        var reg = /^\s*(http:\/\/|https:\/\/).+(\.swf|\.ogg|\.mp4|\.webm|\qq.com)/i;
        ...
        var videoQQFlash = [
            '<iframe class="edui-faked-video" src="https://v.qq.com/iframe/player.html?vid=#{videoUrl}&tiny=0&auto=0" width="#{width}" height="498" allowfullscreen="true"></iframe>',
            '<p><br></p>'
        ].join('');
        ...
        var $urlInput = $('<input type="text" class="block" placeholder=" 请填写视频链接"/>');
        ...
        var $TextContainer =$('<p style="color:#bbbbbb;font-size:12px">目前微信只支持腾讯视频和*.mp4格式视频，如果非这两种格式视频，请把视频上传到腾讯视频然后复制网站(v.qq.com开头的地址)到视频链接</p>')
        var $btnSubmit = $('<button class="right">' + lang.submit + '</button>');
        var $btnCancel = $('<button class="right gray">' + lang.cancel + '</button>');
        $btnContainer.append($btnSubmit).append($btnCancel);
        $content.append($urlInputContainer).append($TextContainer).append($sizeContainer).append($btnContainer);
        ...
        $btnSubmit.click(function (e) {
            ...
            } else if((/qq.com/i).test(url)){
                //腾讯视频
                var $url= url.substring(url.indexOf("r")+1,url.indexOf(".html")).split("/")[2] ;
                console.log($url,'url---------------------')
                html = videoQQFlash.replace(/#{videoUrl}/ig, $url)
                    .replace(/#{width}/ig, '100%')
                    .replace(/#{height}/ig, height);
            }else {
                //其他格式，如swf ogg webm
                console.log(this,'this')
                alert('为了在手机端有更好的体验，请上传MP4格式的视频');
...
_e(function (E, $) {
    ...
    // 百度地图的key
    E.baiduMapAk = 'ep4U9yfgijUmA2zRMjf25Tf6ix96nPvV';
    ...
    E.createMenu(function (check) {
        ...
        $btnSubmit.click(function (e) {
            ...
            if(isDynamic){
                //动态地址
                src = 'http://ueditor.baidu.com/ueditor/dialogs/mapNav/show.html#';
            }else{
                //静态地址
...
_e(function (E, $) {
    ...
    E.fn.renderMenus = function () {
        ...
        var menuIds = editor.config.menus.concat();
        var menuContainer = editor.menuContainer;
